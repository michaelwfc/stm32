...
void IRremote_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Mode=GPIO_Mode_IPU; // GPIO_Mode_Out_IPU  上拉输入, 默认为高电平
	GPIO_Init(IRED_PORT,&GPIO_InitStructure);
	
	// GPIO AFIO 设置中断引脚选择:  只要连接到 EXTI_Line8
	...
	/* 设置NVIC参数 */
	NVIC_InitStructure.NVIC_IRQChannel = EXTI9_5_IRQn;   //打开全局中断 STM32F10X_MD : External Line[9:5] Interrupts
