...
void PWM_Init(void)
{
      ...
      // PB6 , PB7,PB8,PB9 is pwn for 2 motors                                                                    
	  ...
	  Reso = 1/(ARR+1) = 1/100 = 1%
	  Freq =  CN_CNT/(ARR+1)/ (PSC +1) = 72M /100 /36 = 20K
	  Duty = CCR / (ARR + 1)   = CCR/100
	  */
      ...
      TIM_OCInitStructure.TIM_Pulse = TIM_Pulse_init; // CCR 寄存器值
      ...
      TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
      TIM_OCInitStructure.TIM_Pulse = TIM_Pulse_init;
      TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High;
       //TIM4_CH2初始化,OC2
